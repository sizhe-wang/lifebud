...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkCVLIEFWM_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkCVLIEFWM_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkCVLIEFWM_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  const msgs = messages.map((m) => {
    ...
    const safeParts = m.parts.filter((p) => {
      if (!chunkCVLIEFWM_cjs.isToolUIPart(p)) return true;
    ...
    const sanitized = {
      ...
      parts: safeParts.map((part) => {
        if (chunkCVLIEFWM_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkCVLIEFWM_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkCVLIEFWM_cjs.isToolUIPart(nextPart)) {
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkHQHGGAB7_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  const result = chunkCVLIEFWM_cjs.convertToModelMessages(preprocessed);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkCVLIEFWM_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
//# sourceMappingURL=chunk-J4KVR4DZ.cjs.map
//# sourceMappingURL=chunk-J4KVR4DZ.cjs.map