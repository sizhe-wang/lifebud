...
var CoreToolBuilder = class extends chunkDGV2FWB4_cjs.MastraBase {
  originalTool;
  options;
  logType;
  constructor(input) {
    ...
    if (!chunkOZ75PBBK_cjs.isVercelTool(this.originalTool) && input.autoResumeSuspendedTools) {
      ...
      if (chunkVW7YQWDW_cjs.isZodObject(schema)) {
        this.originalTool.inputSchema = schema.extend({
          suspendedToolRunId: zod.z.string().describe("The runId of the suspended tool").optional().default(""),
  ...
  getParameters = () => {
    if (chunkOZ75PBBK_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType, processedSchema) {
    ...
    const execFunction = async (args, execOptions) => {
      ...
      try {
        ...
        if (chunkOZ75PBBK_cjs.isVercelTool(tool)) {
          result = await chunkS65YV6YB_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkS65YV6YB_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            runId: options.runId,
            requestContext: options.requestContext ?? new chunkSMM2C4KH_cjs.RequestContext(),
          ...
          if (resumeData) {
            const resumeValidation = chunkOZ75PBBK_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkS65YV6YB_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkOZ75PBBK_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkOZ75PBBK_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkOZ75PBBK_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      try {
        ...
        const { data, error: error2 } = chunkOZ75PBBK_cjs.validateToolInput(parameters, args, options.name);
        if (error2) {
  ...
  build() {
    ...
    let needsApprovalFn;
    if (chunkOZ75PBBK_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkOZ75PBBK_cjs.Tool) && !chunkOZ75PBBK_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-PPEP5ZHX.cjs.map
//# sourceMappingURL=chunk-PPEP5ZHX.cjs.map