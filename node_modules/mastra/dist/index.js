#! /usr/bin/env node
import { package_default, COMPONENTS, parseComponents, LLMProvider, parseLlmProvider, parseMcp, create, checkForPkgJson, getVersionTag, isGitInitialized, checkAndInstallCoreDeps, interactivePrompt, init, logger, FileService, createLogger, shouldSkipDotenvLoading, DepsService } from './chunk-5TKCXKC7.js';
export { create } from './chunk-5TKCXKC7.js';
...
function formatTable(scorers) {
  ...
  const rows = scorers.map(
    (scorer) => `${(scorer?.name ?? scorer?.id).padEnd(nameWidth)} \u2502 ${pc2.dim(scorer.id.padEnd(idWidth))} \u2502 ${scorer.type.padEnd(typeWidth)} \u2502 ${pc2.dim(scorer.description.padEnd(descWidth))}`
  );
...
var DevLogger = class {
  ...
  ready(host, port, studioBasePath, startTime, https) {
    ...
    console.info("");
    console.info(`${pc2.dim("\u2502")} ${pc2.bold("Studio:")} ${pc2.cyan(`${protocol}://${host}:${port}${studioBasePath}`)}`);
...
async function getMastraPackages2(rootDir) {
  try {
...
async function dev({
  ...
  const mastraPackages = await getMastraPackages2(rootDir);
  const startOptions = { inspect, inspectBrk, customArgs, https: httpsOptions, mastraPackages };
...
var createServer = (builtStudioPath, options) => {
  const indexHtmlPath = join(builtStudioPath, "index.html");
  const basePath = "";
  let html = readFileSync(indexHtmlPath, "utf8").replaceAll("%%MASTRA_STUDIO_BASE_PATH%%", basePath).replace("%%MASTRA_SERVER_HOST%%", options.serverHost || "localhost").replace("%%MASTRA_SERVER_PORT%%", String(options.serverPort || 4111)).replace("%%MASTRA_SERVER_PROTOCOL%%", options.serverProtocol || "http");
  const server = http.createServer((req, res) => {
    const url = req.url || basePath;
    const isStaticAsset = url.includes("/assets/") || url.includes("/dist/assets/") || url.includes("/mastra.svg") || url.includes("/favicon.ico");
    if (!isStaticAsset) {
...
program.command("start").description("Start your built Mastra application").option("-d, --dir <path>", "Path to your built Mastra output directory (default: .mastra/output)").option("-e, --env <env>", "Custom env file to include in the start").action(startProject);
program.command("studio").description("Start the Mastra studio").option("-p, --port <port>", "Port to run the studio on (default: 3000)").option("-e, --env <env>", "Custom env file to include in the studio").option("-h, --server-host <serverHost>", "Host of the Mastra API server (default: localhost)").option("-s, --server-port <serverPort>", "Port of the Mastra API server (default: 4111)").option("-x, --server-protocol <serverProtocol>", "Protocol of the Mastra API server (default: http)").action(studio);
